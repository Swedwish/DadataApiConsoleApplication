...
def _change_max_count(new_max_count: str, con: sqlite3.Connection, cur: sqlite3.Cursor):
    if new_max_count > 20 or new_max_count < 1:
        print(
            f"Invalid max_count value. Expected 1<=max_count<=20, got {new_max_count}"
        )
    else:
...
def change_settings(con: sqlite3.Connection):
    ...
    while True:
        print(
            "Type a setting name you want to change and a new value, (e.g. api_token 1234567890) type '.' to go back. Type 'list' to get list of settings."
        )
        args = input().split()
            ...
            match args[0]:
                ...
                case "max_count":
                    try:
                        ...
                        print(
                            f"Invalid max_count value. Expected 1<=max_count<=20, got {args[1]}"
                        )
                case _:
                    print(
                        f"Unexpected setting name. Choose from 'api_token', 'language' or 'max_count'. Got {args[0]} instead."
                    )
