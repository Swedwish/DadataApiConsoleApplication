            ...
            try:
                if int(answer) in range(len(location_list)):
                    result = dadata.suggest(
                        name="address",
                        query=location_list[int(answer)]["unrestricted_value"],
                        count=max_count,
                        language=language,
                    )
                    if result[0]["data"]["geo_lat"] == None:
                        print(
                            "Sorry, no geographical coordinates are availible for chosen address."
                        )
                    else:
                        print(
                            f"""Geographical latitude of chosen location is {result[0]['data']['geo_lat']}\nGeographical longitude of chosen locantion is {result[0]['data']['geo_lon']}"""
                        )
                    break
                else:
                    print(
                        "I do not understand this input, pick one entry for more info by typing its' number or type dot symbol ('.') to go back."
                    )
            except ValueError:
                print(
                    "I do not understand this input, pick one entry for more info by typing its' number or type dot symbol ('.') to go back."
...
def request(con: sqlite3.Connection, *args):
    ...
    try:
        result = dadata.suggest(
            name="address", query=" ".join(args), count=max_count, language=language
        )
    ...
    print(
        "Pick one entry for more info by typing its' number or type dot symbol ('.') to go back."
    )
